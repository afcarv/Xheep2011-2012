...
// Validações prévias
	...
	//include('/xheep2/tabela_inqext02_andre/arrext01.php');	// .... ....
...
// SELMUN
 function selent($cod,&$nom,&$dis) {
	...
	$ress=mysql_db_query("impresso_ISsite",$sqls); //
		if ($ress) {
...
// 	################################ Cabeçalho #####################
		// printf("<b>início do dito !!!!! mov=%s</b>",$tip_mov);
			...
			printf("<table border=1><tr><td valign=top>Cliente</td><td>Distrito</td>");	 
				foreach ($inq as $iti => $itlis) {
					if ( $arsel[$iti] == 1 ) {
...
<hr>
	...
	[ <a href="http://www.impressoesesolucoes.com/inq.php?xsel=1">Tudo</a> | 
    <a href="http://www.impressoesesolucoes.com/inq.php?xsel=0">Limpar</a> ]
