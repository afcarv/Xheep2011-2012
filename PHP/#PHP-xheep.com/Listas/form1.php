...
// Categorias
 ...
 if ( strlen($codcat) == 0) {
 echo ':';
   foreach ($categorias as $keyx => $catx) {
	 echo ' | <a href="' . $locx . '&codcat=' . $keyx . '">'.$catx . '</a>';
	 }
   } else if ( strlen($codart) != 0  ){ 
			 echo ' | <a href="' . $locx . '&codcat=' . $codcat . '">'. $categorias[$codcat] . '</a>';
	} else { 
			echo " » ". $categorias[$codcat] . " <br />";
	}
	...
	// trata o artigo x
			...
			foreach ($artigos as $codx => $artx) { // percorre os artigos
				if ($artx["cat"] == $codcat ) {   // lista os artigos da categoria $codcat
					// echo '<br />' . $categorias[$artx["cat"]];
					 	 echo ' <br /> - <a href="' . $locx . '&codart=' . $codx . '">'. $artx["nome"] . ' »»»</a>';
						echo '<hr />';
				}
		}
    } 
