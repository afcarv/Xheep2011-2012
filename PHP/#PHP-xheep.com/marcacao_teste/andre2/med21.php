...
printf("<hr />");
	...
	echo "<tr><td>";
	printf("<br> Especialidade:</td>");
		printf("<td><select class=\"sel1\" name=\"%s\" size=1>","afor[esp]"); 
		printf("<OPTION %s value=\"%s\">%s%c",$xs,"","escolha a especialidade",10);
//	if( strlen($cod_pais) == 0) $cod_pais = "PT"; // Por defeito é portugues
	foreach ($aesp as $cesp => $nesp) {
 		if ( $cesp == $afor["esp"] ) $xs = "SELECTED"; else $xs = ""; 
		printf("<OPTION %s value=\"%s\">%s%c",$xs,$cesp,$nesp,10);
		printf("\n*");
	}
	echo "</select>";
	echo "</td></tr>";
	...
	echo "</table>";
 		...
 		printf("<br><br><input type=\"submit\" value=\"%s\">","Seguinte");
		printf("</form>");	
