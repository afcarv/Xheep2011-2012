...
// se comentarmos aqui temos de obrigar antes de inser ou update a validar o e-mail (+/- linha;	else { 
...
function checkEmail(email) {
 if(email.length > 0)  {
      if (email.indexOf(' ') >= 0)
           ...
           alert("Erro: O Endereço (email) não está completo\n Corrija este dado e prima novamente o botão Submeter");
   }
 }
...
if ($tip_mov == "IN" || $tip_mov == "UPD"  ) {  // SE insert ou update
	...
	// validação do e-mail ....
	if (strlen($email) == 0 ) {
				$werr = "<hr><div class=sel3r align=left> - O e-mail tem de ser preenchido</b></div>"; //  erros	
				$erro = 1;
			}
