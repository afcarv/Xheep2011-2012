...
//AQUI NÃO PODE ESTAR A PÔR SEMPRE A 0. TEM DE APANHAR O QUE VEM DOS REFERRALS
//$id_ref=0;
...
function enviaPara($email, $nome) 
{
   ...
   //$mail->CharSet = 'iso-8859-1'; // Charset da mensagem (opcional)
    ...
    $mail->Subject  = "Campanha Impressões e Soluções"; // Assunto da mensagem
	 $mail->Body = 'Caro(a) '.$nome.'.<br>
Agradecemos a sua participação na campanha de personalização de calendários da Impressões e Soluções.
<br>Deve confirmar a validade da sua conta de e-mail seguindo o link: <a href="http://www.impressoesesolucoes.com/teste/confirm.php?email='. $email .'&id_ref=' . $id_ref . '&id='.$id.'"> Validação do e-mail </a>
<br><a href="http://www.impressoesesolucoes.com/teste/campaign.php?tip_mov='.
$new . '&email='. $email .'&id_ref=' . $id_ref . '"> Campanha Impressões e Soluções </a>
<br>Em breve receberá na sua caixa de correio electrónico o resultado da sua participação na campanha.
<br>Aproveite e visite a nossa página web (www.impressoesesolucoes.com) ou a nossa página do facebook (www.facebook.com/impressoesesolucoes) e esteja atento às nossas promoções e serviços.
<br><br>A Equipa da Impressões e Soluções, Lda';
...
if ( $erro == 0 ) {  // SE não há erros impeditivos e atingiu a pag final, continua
	...
	//echo "vai inserir";
		...
		$sql = "insert into $idtab  ( nome,apelido,aniversario,email,opcao,date,id_ref,status) values ('$nome','$apelido','$aniversario', '$email', '$opcao','$data', '$id_ref',$status) ";
